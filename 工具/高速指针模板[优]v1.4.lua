...
--注: 如果使用的是E, F类型请按照以下格式保留副特征(保留一位小数点即可，如果副特征码是0.05~0.063这种就书写为0~0.1即可。)
--原: -1.66055106e249
--改: -1.6
--原: -1.03E-999
--改: -1.0
...
--注: 支持范围副特征使用，使用方法:   
--注: 范围搜索需加上字符串才不会报错
--{"1~99999", 0x24, 16}--副特征码, 偏移, 类型
local Search_array = {
   {"1", 4, A, "测试"},--主特征码, 类型, 内存范围, 功能, 内存地址起始, 内存地址结束
   {2.1, -4, 64}, --副特征码, 偏移, 类型
}
...
local num = LSQ_pointer(Search_array, Modifythe_array)--判断这个的返回值，如果返回的是数量那么就证明修改成功了，如果返回的是false那么就没有修改到
print(num)
