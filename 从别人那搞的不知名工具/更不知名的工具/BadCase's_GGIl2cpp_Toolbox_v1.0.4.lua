...
require('Il2cppApi')
local setIl2cppVersion = gg.choice({
    "✅ Yes", 
    "❌ No"
}, 
    nil, 
    bc.Choice("Set Il2cpp Version", "Do you want to override GGIl2cpp's version detection and set the version manally?", "ℹ️")
)
if setIl2cppVersion ~= nil and setIl2cppVersion == 1 then
    local il2cppVersions = {
        20,
        21,
        22,
        23,
    ...
    local setVersion = gg.choice (
        il2cppVersions,
        nil,
        bc.Choice("Select Il2cpp Version", "Select the Unity Il2cpp version to send to GGIl2cpp", "ℹ️")
    )
    Il2cpp({il2cppVersion = il2cppVersions[setVersion]})
else
    Il2cpp()
end
...
ggil2cppFrontend = {
    ...
    FindFields = function(fieldNames)
		if type(fieldNames) == "string" then
...
ggil2cppEdits = {
    ...
    getGlobalMetadataStrings = function()
        if not ggil2cppEdits.globalMetadataStrings then
            local DumpDIR = gg.EXT_STORAGE .. "/bc/"
            local dump_start
            local dump_end
            local dump_start_address
            local dump_end_address
            for i, v in pairs(gg.getRangesList()) do
                if Il2cpp.globalMetadataStart >= v.start and Il2cpp.globalMetadataStart < v["end"] then
            ...
            gg.dumpMemory(dump_start_address, dump_end_address - 4, DumpDIR)
            dump = DumpDIR .. gg.getTargetPackage() .. "-" .. dump_start .. "-" .. dump_end .. ".bin"
            print(dump)
            readDump = io.open(dump, "r"):read("*a")
            os.remove(dump)
            trim = readDump:match("mscorlib.dll(.+)mscorlib.dll")
            trim = trim:gsub(string.char(0), "\n")
            io.open(DumpDIR .. "dump.cs", "w"):write(trim)
            local ReadedInfo = {}
            ReadDump = io.open(DumpDIR .. "dump.cs", "r"):read("*a")
            local startCapturing = false
            for i in ReadDump:gmatch("(.-)\n") do
                if i == "mscorlib.dll" then
                    startCapturing = true
                end
                if i:find("^[a-zA-Z0-9._<]") and #i > 2 and startCapturing == true then
                    if i:find("[-]") then
                    else
                        table.insert(ReadedInfo, i)
                    end
                end
            end
            ggil2cppEdits.globalMetadataStrings = ReadedInfo
        end
    end,
