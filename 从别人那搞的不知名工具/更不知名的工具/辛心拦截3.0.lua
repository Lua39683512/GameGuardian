...
local		config = chunk()
    ...
    local Namerepetition=0
       ...
       local Ggname=g.name
						...
						local fnamee = CXGG.Outputpath..'/'..'[Load] 辛心.'..Removalofpolypus
						local fnameeee = CXGG.Outputpath..'/'..'[Lua] 辛心.'..Removalofpolypus						
						local fnchar = CXGG.Outputpath..'/'..'[Char] 辛心.'..Removalofpolypus
						local wFilename=Removalofpolypus
						local fnameee = CXGG.Outputpath..'/'..'[Load] 辛心.'
...
--[[															f:write("load_")
      ...
      local stackcs=0
					...
					local menui=1
					local searchNumber=""
						local file = CXGG.Outputpath..'/'..'[Log] 辛心..'..Removalofpolypus
...
debugsethook(hook, '', 1)													
	...
	selected = ggg.choice(options, #options, "CXGG 拦截密码解析\n 选择复制可疑密码\n Resolution["..#scripts.."]")
...
Hookscript="gg.alert('CXGG: 辛心')\n\nfunction Main()\n"
-----启动脚本
Hookscript=Hookscript..'menu = gg.choice({'..Menuname..'"结束脚本 Exit"},nil,"选择开启功能[初心❤ 辛心拦截生成]")'
-----功能名称
