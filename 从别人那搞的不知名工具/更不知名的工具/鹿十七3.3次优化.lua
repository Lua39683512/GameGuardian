...
function Rl_encrypt_d(DATA)
    ...
    DATA = io.w_r(path)
    DATA = DATA:gsub("numparams [^\n]*", "numparams 2"):gsub("is_vararg [^\n]*", "is_vararg 8"):gsub(
               "maxstacksize [^\n]*", "maxstacksize 99"):gsub("linedefined [^\n]*", "linedefined 0"):gsub(
               "lastlinedefined [^\n]*", "lastlinedefined 0")

    DATA = string.dump(load(DATA), true)
    DATA = DATA:gsub(string.char(table.unpack({0, 0, 0, 0, 0, 0, 0, 0, 2, 8, 99})),
               string.char(table.unpack({255, 255, 255, 255, 255, 255, 255, 255, 250, 250, 250})))
