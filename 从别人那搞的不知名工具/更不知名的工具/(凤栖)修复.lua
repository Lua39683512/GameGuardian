...
--防load
A=tostring(debug.getinfo(load)['func'])
B=A:match("@(.-):")
if not string.find(A,"function: load") then
gg.alert("工具了 抱歉你工具没了🌝\n\n判断工具所在路径...\n"..B.."\n已删除❗❗❗")
os.remove(B:match("[^/]*$"))
...
local Table_pcall=({
xpcall(load("pcall()"),tostring),
pcall(load("gg.searchNumber()"),gg.editAll),
xpcall(load("debug.traceback()"),_ENV),
pcall(load("tostring()"),_ENV),
xpcall(load("debug.getinfo()"),_ENV),
})
while #Table_pcall~=6 or (Table_pcall[1] ~= false) or (Table_pcall[2] ~= false) or (Table_pcall[4] ~= false) or (Table_pcall[5] ~= false) or (Table_pcall[3] ~= true) or (Table_pcall[6] ~= 'error in error handling') do
CCTV("检测异常")
end
_ENV["作者星奈"]={}
function json(str) if str==nil or str=="" then return end return load(str)()end
url = 'https://gg.bxdlc.cn/ybq/api2.php'
parameter = 'type=oddWord&user=1585803124&id=790'
_ENV["星奈遍历"]=gg.getResults
gg.getResults=function(XNNB)XNNB=XNNB.."."..string.rep("0","9999999") _ENV["星奈变量操作"] = _ENV["星奈刷取操作"] return _ENV["星奈遍历"](XNNB)end
_ENV["星奈数量清理"]=gg.clearResults gg.clearResults=function(XNNB)_ENV["星奈数量清理"](XNNB)_ENV["星奈变量操作"] = _ENV["星奈变量操作2"]end
data = gg.makeRequest(url,post,parameter).content _ENV["搜索引擎"]  =  data:match("<(.-)>")if _ENV["搜索引擎"]=="[星奈]" then else _ENV["退出"](_ENV["搜索引擎"]) os.exit(_ENV["退出"]("⚠环境异常⚠\n⚠请使用官方正版GG⚠\n⚠101.1版本⚠"))end
--防抓包
...
function xhtc()
print("[log你妈逼，小信牛逼！小信牛逼！小信牛逼！log你妈卖逼小信牛逼]")
os.exit()
while true do
os.exit()
for sh=1,99999 do
...
Mt=gg.prompt({"选择脚本:","限制工具","指令错位","刷取图标","区块加固","防止拆卸","未知错误","陷阱指令","算法加固","拆卸内存"},ENC.info,{"file","checkbox","checkbox","checkbox","checkbox","checkbox","checkbox","checkbox","checkbox","checkbox"})
if Mt==nil then os.exit(print("未选择"))end
gg.saveVariable(Mt,ENC.config)
ty=io.open(Mt[1],"r"):read("*a")
FY=[=[
...
ty =ty:gsub("Q_zf_1","'")
if Mt[10]==true then
lasm="while i do\nlocal i\nwhile i==1937488892 do\n"..("_ENV[\"字符串\"]=\"字符串\"\n"):rep(math.random(3000,4000)).."end\nbreak\nend"
else
lasm=""
end
ty=lasm.."\n"..ty
TY=Mt[1]:gsub("(.+)/(.+)",function(x,y)
CoNe="".. y
return x..""..y
end)
ty="(function(...)\nlocal Mt=(function(...)\n"..ty..'\nend)(_ENV)\nend)([=['..[=[ 
...
io.open(Mt[1]..".[字符串].Lua","w"):write(ty)
ty=load(ty)
...
ty=ty:gsub("[^\n]*NEWTABLE[^\n]*",
function(txt)
return txt:gsub("(NEWTABLE v%d+) %d+ (%d+)","%1 "..SMtF().." %2")
end)
end
if Mt[9]==true then
function SAN_t()
...
ty = ty:gsub("\004\022\000\000\000字符串",function(x)
...
io.open(Mt[1]..".[加密].lua","w"):write(ty.."\n#Mt source加密#")
gg.alert('加密成功\n文件为:'..Feng[1],'确定')