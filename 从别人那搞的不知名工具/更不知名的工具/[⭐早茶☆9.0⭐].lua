...
ZaoCha=gg.prompt({"选择脚本:","限制工具","指令错位","刷取图标","区块加固","防止拆卸","未知错误","陷阱指令","算法加固","拆卸内存"},ENC.info,{"file","checkbox","checkbox","checkbox","checkbox","checkbox","checkbox","checkbox","checkbox","checkbox"})
if ZaoCha==nil then os.exit(print("未选择"))end
gg.saveVariable(ZaoCha,ENC.config)
ty=io.open(ZaoCha[1],"r"):read("*a")
FY=[=[
...
gg.toast("正在进入中")
--防DEGG
while string.find(gg.EXT_CACHE_DIR,"com.hckeam.mjgql") do
end
...
while string.find(gg.EXT_CACHE_DIR,"catch.Art.Tool.seatch") do
end
...
_G["滚"]=function()
_ENV["gg"]["alert"]("早茶：滚犊子","","","")
_G["os"]["exit"]() 
_G["滚"]()
end
for i = -1, -2 do;se = 'Morning tea';end local _  =  debug.getinfo(gg.searchNumber).source ~= "=[Java]" or  not not debug.getupvalue(gg.searchNumber,1,2) local _ = _  == false or (function() _ENV["滚"]()  end)()


--防恶魔
_ENV["debug"]["getinfo"]=function(a)
return _ENV["debug"]["getinfo"]("脚本已退出")
end
...
--符号返回
ty =ty:gsub("Q_zf_1","'")
if ZaoCha[10]==true then
lasm="while i do\nlocal i\nwhile i==QQ1955619037 do\n"..("_ENV[\"消炎水笑死人了\"]=\"牛马玩意笑飞我\"\n"):rep(math.random(3000,4000)).."end\nbreak\nend"
else
lasm=""
end
ty=lasm.."\n"..ty
TY=ZaoCha[1]:gsub("(.+)/(.+)",function(x,y)
CoNe="".. y
return x..""..y
end)
ty="local ZaoCha=(function(...)\n"..ty..'\nend)(ZaoCha)([=['..[=[ 

-- ☨━━━━━━━━━━━━━━━━━━━☨

🅜🅞🅡🅝🅘🅝🅖    🅣🅔🅐

  🌈[ZaoCha · Enc - Lua - Script]☁️
  
     ➖➖➖➖➖➖➖➖
     
        Encrypted version：𝟡.𝟘
...
HXZaoCha[#HXZaoCha+1]=i
end
str=table.concat(ZaoChaHX(HXZaoCha),"\n")
return "maxstacksize "..max.."\n"..table.concat(JMP3,"\n").."\nTFORLOOP v248 GOTO[1955]\nLOADK v244 1\nLOADK v245 1\nLOADK v246 1\nLOADBOOL v249 1\nTFORLOOP v248 :goto_1000000\n"..table.concat(ZaoChaHX(JMP5),"\n").."\n"..str.."\n"..table.concat(ZaoChaHX(JMP6),"\n").."\n"
else
...
if ZaoCha[3]==true then
if ZaoCha[4]==true then
ty=JMPHX(ty,true)
else
ty=JMPHX(ty)
end
...
if ZaoCha[6]==true then
ty=ty:gsub('(\n%s*RETURN [^\n]*)','%1\nRETURN v250..v250;\x4A\x53',1)
ty=ty:gsub('(\n%s*TAILCALL [^\n]*)','%1\nRETURN v250..v250;\x4A\x53',1)
end
if ZaoCha[7]==true then
QKCW=[=[
.upval u1 "" ; u1
.upval u9 "" ; u2
.upval u10 "" ; u3
...
if ZaoCha[8]==true then
local SZaoChaF=function()
return math.random(100,1100)
end
ty=ty:gsub("[^\n]*NEWTABLE[^\n]*",
function(txt)
return txt:gsub("(NEWTABLE v%d+) %d+ (%d+)","%1 "..SZaoChaF().." %2")
end)
end
if ZaoCha[9]==true then
function SAN_t()
...
io.open(ZaoCha[1]..".[加密完成].lua","w"):write(ty.."\n#ZaoCha加密#")
gg.alert('加密成功\n文件为:'..ZaoCha[1],'确定')