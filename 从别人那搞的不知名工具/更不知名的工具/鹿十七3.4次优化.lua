        ...
        if types == "table" then
            for kk, vv in pairs(v) do
                DATA = DATA:gsub(k .. "%s*%.%s*" .. kk,
                           "_ENV[(function()::Rl3::if nil then goto Rl3 end return '" .. k .. "'end)()][(function()::Rl3::if nil then goto Rl3 end return '" .. kk .. "'end)()]")
            end
        end
    end
    DATA = DATA:gsub("%s+", " ")
...
function Rl_encrypt_c(DATA)
    ...
    function string1(text)
        data = ""
        for k, v in pairs({text:byte(1, -1)}) do
            v = tostring(v)
            len = string.len(v)
            if len == 1 then
                v = "00" .. v
        ...
        text = "(function()::Rl2::if nil then goto Rl2 end return " .. char .. "('" .. data .. "')end)()"
        return text
    end
    ...
    DATA = gsub .. "=string.gsub\n" .. DATA
    DATA = char .. "=function(text)while text do return tostring(" .. gsub .. "(text,'...',string.char)) end end\n" ..
               DATA
